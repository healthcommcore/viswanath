...
class plgEditorWysiwygPro3 extends JPlugin {
	...
	//function botWysiwygPro3EditorEditorArea( $name, $content, $hiddenField, $width, $height, $col, $row ) {
		...
		if ($this->params->get( 'image_manager', 1 ) ) {
			...
			$img->URL = $editor->stripTrailingSlash($url).$editor->addLeadingSlash($editor->stripTrailingSlash($this->params->get( 'image_folder', '/images/stories/')));
			$img->dir = $editor->stripTrailingSlash($path).$editor->addLeadingSlash($editor->stripTrailingSlash($this->params->get( 'image_folder', '/images/stories/')));
		...
		if ($this->params->get( 'document_manager', 1 ) ) {
			...
			$doc->URL = $editor->stripTrailingSlash($url).$editor->addLeadingSlash($editor->stripTrailingSlash($this->params->get( 'document_folder', '/images/stories/')));
			$doc->dir = $editor->stripTrailingSlash($path).$editor->addLeadingSlash($editor->stripTrailingSlash($this->params->get( 'document_folder', '/images/stories/')));
		...
		if ($this->params->get( 'media_manager', 1 ) ) {
			...
			$med->URL = $editor->stripTrailingSlash($url).$editor->addLeadingSlash($editor->stripTrailingSlash($this->params->get( 'media_folder', '/images/stories/')));
			$med->dir = $editor->stripTrailingSlash($path).$editor->addLeadingSlash($editor->stripTrailingSlash($this->params->get( 'media_folder', '/images/stories/')));
		...
		if ($this->params->get( 'user_folders', 0 )) {
			...
			if ($gid) {
				...
				$basedir = $editor->stripTrailingSlash($path).$editor->addLeadingSlash($editor->stripTrailingSlash($this->params->get( 'user_folder', '/images/users/')));
		...
		$editor->allowedMediaExtensions = $this->params->get( 'allowedMediaExtensions', '.asf, .asx, .flv, .h264, .mov, .mp3, .mp4, .swf, .wax, .wma, .wmv, .wvx, .wpl, .xspf' );
		...
		if ($values = $this->params->get( 'sizes', '' )) {
			...
			if (strstr($search, 'strict')||strstr($search, 'xhtml 2')||strstr($search, 'xhtml 1.1')) {
				for ($i=0; $i<$num; $i++) {
					$k = preg_replace("/[^0-9]/", '', $values[$i]);
					if (!preg_match("/[0-9][a-zA-Z]/", $values[$i])) {
						$values[$i] = $k.'px';
					}
					$array[$values[$i]] = $k;
				}
				...
				for ($i=0; $i<$num; $i++) {
					$array[trim($values[$i])] = trim($values[$i]);
				}
			}
		...
		// default content
		$editor->emptyValue = $this->params->get( 'emptyValue', 'auto' );
		$editor->newCellInners = $this->params->get( 'newCellInners', 'auto' );
		
		if ($editor->emptyValue == 'empty') $editor->emptyValue = '';
		if ($editor->newCellInners == 'empty') $editor->newCellInners = '';
		
		// language
		$htmlLang = $this->params->get( 'htmlLang', '' );
		$editor->htmlLang = empty($htmlLang) ? strtolower($language->getTag()) : $htmlLang;
		...
		// jsBookmarkLinks
		$editor->jsBookmarkLinks = $this->params->get( 'jsBookmarkLinks', false );
		...
		// dirty hack to fix issues with JTabs
		$editor->addOutputBelow('<script type="text/javascript">
/*<![CDATA[ */
WPro.events.addEvent(window, \'load\', function(){
	if (!WPro.isIE && !WPro.##name##.editWindow.getSelection()) {
		WPro.##name##.start();
	}
});
/*]]>*/
	...
	function onGetInsertMethod($name) {
		...
		$js= "function jInsertEditorText( text, editor ) {
			WPro.editors[editor].insertAtSelection( text );
		}";
