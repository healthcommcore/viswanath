...
class Manager extends JContentEditor{
		/*
		*  @var varchar
		*/
        var $_base = null;
		...
		*/
        ...
        function getBaseURL(){
			global $mainframe;
			$site = $mainframe->isAdmin() ? $mainframe->getSiteURL() : JURI::base();
			return Utils::makePath( $site, $this->_base );
        }
		/**
		...
		*/
		function getRootDir(){
			...
			$base 	= $params->get( 'dir', 'images/stories' );
			...
			if( $this->isSuperAdmin() ){
				...
				$base = preg_replace( array( '/\$id/', '/\$username/', '/\$usertype/' ), array( $this->_id, $this->_username, $this->_usertype ), $base );
			}
						
			if( $params->get( 'user_restriction_level' ) != '99' ){ 
				if( !$this->authCheck( intval( $params->get( 'user_restriction_level', '18' ) ) ) ){
					$base = Utils::makePath( $base, $this->getUserPath() );
				}
			}
			$full = Utils::makePath( JPATH_SITE, $base );
			if( !JFolder::exists( $full ) ){
				$this->createFolder( $full );
			}
			$base = JFolder::exists( $full ) ? $base : 'images/stories';
			return ( substr( $base, 0, 1) == '/' ) ? substr( $base, 1 ) : $base;
		}
		/**
         * Get the current users restricted path if any
         * @return string path
         */
		function getUserPath(){
			jimport('joomla.filesystem.file');
			$params = $this->getPluginParams();
			$path	= $params->get( 'user_restriction_path', '' );
			
			$path = preg_replace( array( '/\$id/', '/\$username/', '/\$usertype/' ), array( $this->_id, $this->_username, $this->_usertype ), $path );
			
			return JFile::makeSafe( $path );	
		}
		/**
		...
		*/
		function getFileTypes(){
			if( !isset( $this->_filetypes ) ){
				$this->_filetypes = $this->_params->get( 'filetypes', 'jpg,jpeg,gif,png' );
			}
        ...
        */
		...
		function getStdActions(){			
			...
			if( $this->getAuthOption('upload', '18') ){
				$this->addAction( 'upload', '', '', JText::_('Upload') );
				$this->setXHR( array( &$this, 'uploadFiles' ) );
			}
			if( $this->getAuthOption('new_folder', '18') ){
				$this->addAction( 'new_folder', '', '', JText::_('New Folder') );
				$this->setXHR( array( &$this, 'newFolder' ) );
			}
		}
		...
		function getStdButtons(){
			if( $this->getAuthOption('folder_delete', '21') ){
				$this->addButton( 'folder', 'delete', '', '', JText::_('Delete Folder') );
				
				$this->setXHR( array( &$this, 'renameFolder' ) );
			}
			if( $this->getAuthOption('folder_rename', '21') ){
				$this->addButton( 'folder', 'rename', '', '', JText::_('Rename Folder') );
				
				$this->setXHR( array( &$this, 'deleteFolder' ) );
			}
			if( $this->getAuthOption('file_rename', '21') ){
				$this->addButton( 'file', 'rename', '', '', JText::_('Rename File') );
				
				$this->setXHR( array( &$this, 'renameFile' ) );
			}
			if( $this->getAuthOption('file_delete', '21') ){
				$this->addButton( 'file', 'delete', '', '', JText::_('Delete Files'), true );
				
				$this->setXHR( array( &$this, 'deleteFiles' ) );
			}
			if( $this->getAuthOption('file_move', '21') ){
				...
				$this->setXHR( array( &$this, 'copyFile' ) );
				$this->setXHR( array( &$this, 'moveFile' ) );
			}
		...
		function getEditorOption( $key, $default='' ){
            ...
            foreach( $files as $file ){
                ...
                if( JFile::exists( $path ) ){
                    if( @!JFile::delete( $path ) ){
						...
						$this->_result = $this->fireEvent('onDeleteFile', array( rawurldecode( $file ) ) );
					}
				}
            }
        ...
        function deleteFolder( $relative ){
            ...
            if( Utils::countFiles( $folder, '^[(index.html)]' ) != 0 || Utils::countDirs( $folder ) != 0 ){
                ...
                if( @!JFolder::delete( $folder ) ){
					...
					$this->_result = $this->fireEvent('deletefolder');
				}
            }
            return $this->returnResult();
        }
        ...
        */
        function renameFile( $src, $dest ){			            
            ...
            if( !JFile::move( $src, $dest ) ){
				...
				$this->_result = $this->fireEvent('renamefile');
			}
            return $this->returnResult();
        }
        /*
        ...
        * @return array $error
        */
        function renameFolder( $src, $dest ){
            jimport('joomla.filesystem.folder');
			$src = Utils::makePath( $this->getBaseDir(), rawurldecode( $src ) );
            ...
            if( !JFolder::move( $src, $dest ) ){
				...
				$this->_result = $this->fireEvent('renamefolder');
			}
			return $this->returnResult(); 
        }
        /*
        ...
        function copyFile( $files, $dest ){			
            ...
            foreach( $files as $file ){
				...
				if( !JFile::copy( $filesrc, $filedest ) ){
					...
					$this->_result = $this->fireEvent('copyfile');
				}
            }
            return $this->returnResult();
        }
        ...
        function moveFile( $files, $dest ){			
            ...
            foreach( $files as $file ){
                ...
                if( !JFile::move( $filesrc, $filedest ) ){
					...
					$this->_result = $this->fireEvent('movefile');
				}
            }
            return $this->returnResult();
        }
