...
var Uploader = new Class({
	...
	options: {
		...
		errorCodes : {
			...
			400: " Upload error!",
			409: " Unable to process file!",
			415: " Unsupported file type!",
			412: " Invalid target!",
			417: " Unsupported file size!"	
		},
		buttonLabels : {
			browse: 'Add File...',
			clear: 'Clear Queue'
	...
	initialize: function(el, options){
		...
		if(this.options.method == 'flash'){
			...
			}, this.initializeFlash.bind(this), this.onLoadFail.bind(this), {
	...
	initializeIframe : function(){
		...
		new IframeUploader(this.element, {
			onComplete : function(o){
					if(o.error){
						$$('span.queue-text').setText($$('span.queue-text').getText() + ' - ' + o.text);
					...
					this.fireEvent('onComplete', [o.text]);
	...
	onError: function(name, size, error) {
		var msg = "Upload failed - " + this.options.errorCodes[error.toInt()];
		var index = this.uploadStatus(name, size, 100);
...
IframeUploader.implement(new Options, new Events);