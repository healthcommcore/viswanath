	...
	onBlurEditableSelectInput : function() {
		...
		if (se) {
			if (se.previousSibling.value != '') {
				jce.dom.addSelect(se.id, se.previousSibling.value, se.previousSibling.value);
				jce.dom.getSelect(se.id, se.previousSibling.value);
				//addSelectValue(document.forms[0], se.id, se.previousSibling.value, se.previousSibling.value);
				//selectByValue(document.forms[0], se.id, se.previousSibling.value);
			} else
				//selectByValue(document.forms[0], se.id, '');
				jce.dom.getSelect(se.id, '');
...
TinyMCE_Utils = {
	...
	fillClassList : function(id) {
		...
		if(cl.length > 0){
			...
			lst.options[lst.options.length] = new Option(tinyMCEPopup.getLang('jcebox', 'JCE Lightbox'), 'jcebox');
			lst.options[lst.options.length] = new Option(tinyMCEPopup.getLang('jce_tooltip', 'JCE Tooltip'), 'jce_tooltip');
	
			tinymce.each(cl, function(o) {
				lst.options[lst.options.length] = new Option(o.title || o['class'], o['class']);
			});
		}else{
			tinyMCEPopup.dom.remove(tinyMCEPopup.dom.getParent(id, 'tr'));
		}
	}
}
/**
