...
var Manager = Plugin.extend({
	moreOptions : function(){
		return {
			...
			upload:			'flash',
			onInit:			Class.empty,
			onDeleteFiles: 	Class.empty,
			onDeleteFolder: function(node){
				if(this.treeLoaded){
					// Remove tree node
					this.tree.removeNode(node);
				}	
			}.bind(this),
			onRenameFile: 	Class.empty,
		...
		switch(name){
			...
			case 'view':
				...
				if(viewable.split(',').contains(string.getExt(name))){
					...
					if(/\.(jpeg|jpg|gif|png)/i.test(name)){
						new imagePreview(name, url, {
							width: 400,
							height: 400
						});	
					}else if(/\.(avi|wmv|wm|asf|asx|wmx|wvx|mov|qt|mpg|mp3|mp4|mpeg|swf|flv|xml|dcr|rm|ra|ram|divx)/i.test(name)){
			...
			case 'upload':
						...
						this.uploader = new Uploader($('upload-input'), {
							...
							onStart : function(files){
								...
								$E('form').getElementsBySelector('input[name^=upload-],select[name^=upload-]').each(function(el){
									...
									if(!name || value === '' || value === false) return;
									var qs = function(val){
				...
				break;
			case 'new_folder':
				this._dialog['new_folder'] = new Prompt(tinyMCEPopup.getLang('dlg.new_folder', 'New Folder'), {
					text: tinyMCEPopup.getLang('dlg.name', 'Name'),
					onConfirm: function(){
						var folder = $('prompt').value;
						if(folder){
							this.setMessage(tinyMCEPopup.getLang('dlg.message_load', 'Loading...'), 'load');
							this.json('newFolder', [dir, string.safe(folder)], function(o){		  															  
								if(!o.error){
									this.fireEvent('onNewFolder');
									this.refreshList();
									this._dialog['new_folder'].close();
								}else{
			...
			// Paste the file
			case 'paste':
				var fn = (this._paste == 'copy') ? 'copyFile' : 'moveFile';
				this.setMessage(tinyMCEPopup.getLang('dlg.message_load', 'Loading...'), 'load');
				...
				this._dialog['confirm'] = new Confirm(msg, {
					onConfirm: function(){
						...
						this.json(fn, list, function(o){		  															  
							if(!o.error){
								if(fn == 'deleteFolder'){
									this.fireEvent('onDeleteFolder', list);
								}else{
									this.fireEvent('onDeleteFile');	
								}
								this.refreshList();
							}else{
								this.raiseError(o.error);
