...
(function() {
	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			ed.onBeforeSetContent.add(function(ed, o) {
				...
				h = h.replace(/<script[^>]*>\s*write(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia|DivX)\(\{([^\)]*)\}\);\s*<\/script>/gi, function(a, b, c) {
					...
					return '<img class="mceItem' + b + '" title="' + ed.dom.encode(c) + '" src="' + url + '/img/trans.gif" width="' + o.width + '" height="' + o.height + '" style="' + o.style + '" />'
				});
			...
			ed.onPreProcess.add(function(ed, o) {
				...
				if (o.get) {
					each(dom.select('IMG', o.node), function(n) {
						...
						if (ed.getParam('media_use_script', false)) {
							if (isMediaElm(n))
						...
						switch (n.className) {
							case 'mceItemFlash':
								...
								cb = ed.getParam('media_codebase_flash', 'http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0');
							...
							case 'mceItemShockWave':
								...
								cb = ed.getParam('media_codebase_shockwave', 'http://download.macromedia.com/pub/shockwave/cabs/director/sw.cab#version=8,5,1,0');
							...
							case 'mceItemRealMedia':
								...
								cb = ed.getParam('media_codebase_realplayer', 'http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,0,0');
		...
		_buildObj : function(o, n) {
			...
			each (p, function(v, k) {
				if (!/^(width|height|codebase|classid|style)$/.test(k))
					dom.add(ob, 'span', {mce_name : 'param', name : k, '_value' : v});
			});
			// Delete style values for embed element
			delete(p.style);
		...
		_createImg : function(cl, n) {
			...
			each(['id', 'name', 'width', 'height', 'flashvars', 'src', 'wmode'], function(na) {
