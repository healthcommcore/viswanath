...
class PluginsViewPlugins extends JView
{
	function display( $tpl = null )
	{
		...
		$filter_order		= $mainframe->getUserStateFromRequest( "$option.$client.filter_order",		'filter_order',		'p.type',	'cmd' );
		...
		$filter_access		= $mainframe->getUserStateFromRequest( "$option.$client.filter_access", 	'filter_access', 	0, 		    'cmd' );
		...
		$query = 'SELECT p.*, u.name AS editor, g.name AS groupname'
			...
			. ' LEFT JOIN #__core_acl_aro_groups AS g ON g.id = p.access'
			. $where
			. ' GROUP BY p.id'
			. $orderby
		;
		...
		//access filter
		$lists['access'] = JCEHelper::accessList( 'filter_access', $filter_access, 1, 'document.adminForm.submit();' );
